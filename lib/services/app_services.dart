...
Future<void> saveLocation(
    String? id, String locationName, Map<String, dynamic> data) async {
...
Future getSunTimes(double lat, double long) async {
  var result = await Dio().post(
      Constants.baseUrl + "lat=" + lat.toString() + "&lng=" + long.toString());
...
Future<List<FavModel>> getFavLocations(BuildContext context) async {
  ...
  CollectionReference refSM = FirebaseFirestore.instance
      .collection('users')
      .doc(sp.uid)
      .collection("locations");

  return await refSM.get().then((event) => event.docs
      .map((e) => FavModel.fromJson(e.data() as Map<String, dynamic>))
      .toList());
}
